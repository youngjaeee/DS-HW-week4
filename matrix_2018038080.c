...
{
    _sleep(1000); // seed값 차이를 두기 위해 0.5초 대기
...
int addition_matrix(int** matrix_a, int** matrix_b, int row, int col)
{
    ...
    for (int i = 0; i < row; i++)
    {
        for (int j = 0; j < col; j++) // 이중 for문을 통해 배열 내 모든 요소에 대해
        {
            ...
            if (matrix_add[i][j] <= -1 || matrix_add[i][j] >= 39) // 더한 값이 가능한 범위를 초과한 값일 경우
            {
                /* 후처리 검사 */
...
int subtraction_matrix(int** matrix_a, int** matrix_b, int row, int col)
{
    ...
    for (int i = 0; i < row; i++)
    {
        for (int j = 0; j < col; j++) // add_matrix와 마찬가지로 이중 for문을 통해
        {
            ...
            if (matrix_sub[i][j] < -20 || matrix_sub[i][j] >20) // 차의 값이 가능한 범위를 초과한 값일 경우 메시지 출력
            {
                /* 후처리 검사 */
...
int transpose_matrix(int** matrix, int** matrix_t, int col, int row) // 전치행렬을 구하는 함수로 본행렬과 전치행렬 이중포인터, 행과 열 변수 매개변수로 받음
{
    for (int i = 0; i < col; i++)
    {
        for (int j = 0; j < row; j++) // 이중 for문을 통해 본행렬에 속하는 모든 요소에 대해
        {
            ...
            if (matrix_t[i][j] <= -1 || matrix_t[i][j] >= 20) // 전치행렬값이 가능한 범위를 초과하는 경우
            {
                // 후처리 검사
    ...
    for (i = 0; i < row; i++)
    {
        for (j = 0; j < row; j++)
        {
            for (k = 0; k < col; k++) // 삼중 for문을 통해 matrix_a와 matrix_a_t에 대한 행렬곱 구함
            {
                ...
                if (matrix_mul[i][j] <= -1) // 곱한 값이 정상적인 범위를 초과하는 경우
                {
                    // 후처리 검사
                    printf("multiply 중 오류가 발생했습니다. multiply 함수 종료\n"); // 오류 메시지 출력
                    return -1;// NOT O.K. 
                }
            }
        }
